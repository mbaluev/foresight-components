...
function NotifyWarning(title, text, time, sticky) {
    ...
    Notify(title, text, '/asyst/img/notify-icon-warning.png', time, sticky, cssClass);
}
Loader = {};
...
LinkService.CreateByDataAttr = function () {
    $(document)
        .off('click contextmenu', 'a[data-save-tab-and-go]', LinkService.Handler)
        .on('click contextmenu', 'a[data-save-tab-and-go]', LinkService.Handler)
        
};

document.addEventListener("DOMContentLoaded", LinkService.CreateByDataAttr);
...
function ViewClick(view, item, column, event) {
    ...
    if (entity != null && entity.isViewProcessLink) {
        ...
        var resPath = '/asyst/' + Asyst.Workspace.views[viewName].entity.name + '/form/auto/' + item[Asyst.Workspace.views[viewName].entity.idName] + '?mode=view';
        ...
        if (event && event.hasOwnProperty('ctrlKey') && event.ctrlKey === true) {
            ...
            window.location.href = saveTabAndLink(resPath);
        }
    }
}
...
function ViewExport(viewName, result) {
    ...
    function sheetData(data, columns, groups) {
        ...
        for (var c = 0; c < columns.length; c++) {
            ...
            fillCell(ws, c, 0, text, bold);

            ws['!cols'].push({wch: columns[c].width / 8}); //8 - это магически подобранное число,
        }
        
        var lastColumnNumber = columns.length;
        //Group column titles 
        for (var groupNumber = 0; groupNumber < groups.length; groupNumber++) {
            var grp = groups[groupNumber];
            var groupTitle = grp.title;

            if (!groupTitle) continue;

            fillCell(ws, lastColumnNumber++, 0, groupTitle, bold);
        }

        function fillCell(ws,columnNumber, rowNumber, text, style) {
            var cell = { v: text, t: 's', s: style };
            var cell_ref = XLSX.utils.encode_cell({ c: columnNumber, r: rowNumber });
            ws[cell_ref] = cell;
        }
