(function ($) {
    ...
    var makeHighChart, makeTreemap, makePieChart;
    makeHighChart = function (chartType, extraOptions) {
        return function (pivotData, opts) {
            var agg, colKey, colKeys, categories, defaults, groupByTitle, h, hName, hAxisTitle, series, rowKey, rowKeys, title, vAxisTitle, _i, _j, _len, _len1;
            defaults = {
                $el: $(document),
                localeStrings: {
                    vs: "vs",
            ...
            for (_i = 0, _len = rowKeys.length; _i < _len; _i++) {
                ...
                h = rowKeys[_i];
                hName = h.join("-");
                if (!hName) hName = opts.localeStrings.series + " " + (_i + 1);
                series.push({
                    name: hName,
                    data: rowData
                });
			...
			vAxisTitle = pivotData.colAttrs.map(titleFormatter).join(",");
	...
	return function (pivotData, opts) {
            ...
            //series
            series = [{name: pivotData.aggregatorName, data:[]}];
            
            for (var c in pivotData.colTotals){
                if (pivotData.colTotals.hasOwnProperty(c)){
                    series[0].data.push({name:c, y:pivotData.colTotals[c].value()});
                }
            }
    ...
    return $.pivotUtilities.highchart_renderers = {
	        "График": makeHighChart("line"),
        "Гистограмма": makeHighChart("bar"),
        "Гистограмма с накоплением": makeHighChart("bar", { plotOptions: { series: { stacking: 'normal' } }, }),
        "Столбчатая диаграмма": makeHighChart("column"),        
        "Столбчатая диаграмма с накоплением": makeHighChart("column", { plotOptions: { series: { stacking: 'normal' } }, }),
        "Диаграмма с областями": makeHighChart("area", { plotOptions: { series: { stacking: 'normal' } }, }),
		"Круговая диаграмма": makePieChart(),
